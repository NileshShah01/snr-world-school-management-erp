...
async function handleTimetableUpload(event) {
    ...
    try {
        ...
        // 1. Encode to Base64 (no Firebase Storage).
        //    PDFs/images > ~700 KB are rejected by ImageStorage.
        ...
        await schoolDoc('timetables', classId).set(
            withSchool({
                className: className,
                sessionId: sessionId,
                fileData: saved.dataUri,    // Base64 data URI
                fileUrl: saved.dataUri,     // backward-compat alias for legacy readers
                fileName: saved.name,
                fileMime: saved.mime,
