...
const ReportCardTool = {
    ...
    async processReportCard(studentId, examId, sessionId, templateType = 'himalayan') {
        try {
            ...
            // 7. Encode PDF as Base64 and store inside Firestore (no Firebase Storage).
            //    PDFs > ~700 KB are rejected – school PDFs are typically well under this.
            const pdfFile = new File([pdfBlob], fileName, { type: 'application/pdf' });
            const saved = await window.ImageStorage.saveFile(pdfFile, {
                fieldName: 'Report Card PDF',
